...
void main() {
  // 通过顶点对应的uv（x, y）决定每个像素的颜色
  // gl_FragColor = vec4(vUv, 1.0, 1.0);

  // 利用uv实现渐变效果(vUv.x: 从左到右；vUv.y：从下到上; 反向使用 1.0 - vUv.x / vUv.y)
  ...
  // 设置边界 step(edge, x) x > edge, 返回1.0； 反之0.0
  // float strength = mod(vUv.y * 10.0, 1.0);
  ...
  // strength *= 1.0 - step(0.5, distance(vUv, vec2(0.5)) + 0.1);
  // gl_FragColor = vec4(strength, strength, strength, 1.0);
